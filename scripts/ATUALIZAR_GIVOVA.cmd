...
echo Givova Monitor - recuperacao para 1.5.1
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$p = Start-Process -FilePath 'powershell.exe' -Verb RunAs -Wait -PassThru -ArgumentList ('-NoProfile -ExecutionPolicy Bypass -File \"' + $env:RECOVERY_PS1 + '\"'); exit $p.ExitCode"
set "RESULT=%ERRORLEVEL%"

echo.
if "%RESULT%"=="0" (
  echo Recuperacao concluida: Givova Monitor 1.5.1 ativo e report confirmado.
  pause
  exit /b 0
)
if "%RESULT%"=="2" (
